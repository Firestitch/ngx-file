export class Exif {
  ...
  public base64ToArrayBuffer(base64, contentType) {
  ...
  public getImageData(img, callback) {
      const handleBinaryFile = (binFile) => {
          ...
          if (this.isXmpEnabled) {
             var xmpdata= this.findXMPinJPEG(binFile);
             img.xmpdata = xmpdata || {};               
          }
          if (callback) {
              callback.call(img);
      ...
      if (img.src) {
          if (/^data\:/i.test(img.src)) { // Data URI
              var arrayBuffer = this.base64ToArrayBuffer(img.src, null);
  ...
  */
  public getNextIFDOffset(dataView, dirStart, bigEnd){
  ...
  public readThumbnailImage(dataView, tiffStart, firstIFDOffset, bigEnd){
      // get the IFD1 offset
      ...
      var thumbTags: any = this.readTags(dataView, tiffStart, tiffStart + IFD1OffsetPointer, this.IFD1Tags, bigEnd)
  ...
  public readEXIFData(file, start) {
